...
const imageUrls = Array.from({ length: 50 }, (_, i) => `https://picsum.photos/800/600?random=${i}`)
      ...
      <div className="pt-32 pb-8">
        {viewMode === "float" ? (
          ...
          <div className="relative" style={{ 
            height: `${Math.ceil(imageUrls.length / 5) * 500}px`,
            paddingBottom: '100vh'
          }}>
            {imageUrls.map((url, index) => (
              <div
                ...
                className="absolute w-[300px] h-[400px] rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] dark:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden group"
                style={{
                  left: `${10 + (index % 5) * 18}%`,
          ...
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {imageUrls.map((url, index) => (
              <div
                key={index}
                className="relative aspect-square rounded-2xl overflow-hidden group shadow-lg dark:shadow-gray-800/50"
              >
                <img
                  src={url}
                  alt={`Gallery Image ${index}`}
